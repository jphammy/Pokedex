...
extension FileManager {
    // MARK: Library Directory
      ...
      // stackoverflow.com/questions/41162610/create-directory-in-swift-3-0
    ...
    func save<T: Encodable>(object: T, to baseUrl: URL, withId id: String, fileType: String) -> Bool {
        let url = baseUrl.appendingPathComponent("\(id).\(fileType)")
        do {
            let data = try JSONEncoder().encode(object)
            let _ = try data.write(to: url, options: .noFileProtection)
            Log.info("Object saved at \(baseUrl.path)")
            return true
        } catch {
            Log.info("Problem saving object to \(baseUrl.path)")
            return false
        }
    }
    
    func read(withId id: String, at baseUrl: URL, fileType: String) -> Data? {
        let url = baseUrl.appendingPathComponent("\(id).\(fileType)")
